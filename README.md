...
- **Undefined:** значение переменной, которой не было присвоено значение; также результат отсутствия возвращаемого
  значения в функции.
- **Null:** преднамеренно заданное «отсутствие значения»; используется для явного указания, что переменная не содержит
  значимого значения.
